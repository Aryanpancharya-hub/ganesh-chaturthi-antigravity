...
class MockParvatiMata:
    ...
    def update_catch_point(self):
        if self.reach_up:
            self.catch_point.x = self.pos.x + self.facing * 18
            self.catch_point.y = self.pos.y - 68
        else:
            self.catch_point.x = self.pos.x + self.facing * 26
            self.catch_point.y = self.pos.y - 42
...
class SimulationRewardSystem:
    ...
    def attempt_catch(self):
        ...
        if is_matched and not self.runner.is_caught:
            ...
            # Stage 2: Offering modak flight to sanctum altar
            def on_arrive():
...
def test_rewards_and_agility():
    ...
    ground_y = 520.0
    parvati = MockParvatiMata(200.0, ground_y)
